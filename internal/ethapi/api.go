...
func (s *BundleAPI) CallBundle(ctx context.Context, args CallBundleArgs) (map[string]interface{}, error) {
	...
	for i, tx := range txs {
		...
		if err != nil {
			log.Info("Dog::1:: Failed at ApplyTransactionWithResult")
			// Don't let a particular error to fail the whole bundle
			continue
		...
		from, err := types.Sender(signer, tx)
		if err != nil {
			log.Info("Dog::2:: Failed at types.Sender")
			// Don't let a particular error to fail the whole bundle
			continue
			// return nil, fmt.Errorf("err: %w; txhash %s", err, tx.Hash())
		}
...
// The sender is responsible for signing the transactions and using the correct
// nonce and ensuring validity
func (s *BundleAPI) CallBundleWithMempool(ctx context.Context, args CallBundleArgs) (map[string]interface{}, error) {
	txs, err := s.b.GetPoolTransactions()
	if err != nil {
		return nil, fmt.Errorf("could not retrieve from mempool: %w", err)
	}
	var encodedTxs []hexutil.Bytes

	for _, tx := range txs {
		var encodedTx hexutil.Bytes
	...
	args.Txs = encodedTxs
	args.BlockNumber = rpc.BlockNumber(s.b.CurrentBlock().Number.Uint64())
	args.StateBlockNumberOrHash = rpc.BlockNumberOrHashWithNumber(args.BlockNumber)
	return s.CallBundle(ctx, args)
